...
| `codeScanningDelegatedAlertDismissal` _string_ | CodeScanningDelegatedAlertDismissal enables users to dismiss code scanning alerts.<br />When enabled, users with appropriate permissions can dismiss alerts that don't require action. |  | Enum: [enabled disabled not_set] <br /> |
| `codeScanningOptions` _[CodeScanningOptions](#codescanningoptions)_ | CodeScanningOptions configures advanced code scanning options. |  |  |
