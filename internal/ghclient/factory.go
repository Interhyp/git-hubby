...
// InvalidateClient removes a single client from the cache by cacheKey, forcing recreation on next request.
...
// InvalidateCredentials removes the cached credentials for the given secret name and evicts all
// clients that were created from those credentials, so they are recreated on the next request.
func (m *CachingGitHubClientFactory) InvalidateCredentials(secretName string) {
	...
	logf.Log.Info("Invalidated credentials and clients for secret", "secretName", secretName)
}
